# Intersmash Applications - WildFly SAML Adapter + Keycloak + EJB
...
```bash
export SSO_URL=http://127.0.0.1:8080
export SSO_REALM=saml-realm
export SSO_USERNAME=admin
export SSO_PASSWORD=admin
export SSO_SAML_LOGOUT_PAGE='/index.jsp'
export SCRIPT_DEBUG=true
export LOGGING_SCRIPT_DEBUG=true
export SSO_DISABLE_SSL_CERTIFICATE_VALIDATION=true
#./target/server/bin/standalone.sh -Djboss.http.port=8180
export JBOSS_HOME=$PWD/target/server
export WILDFLY_PUBLIC_BIND_ADDRESS=127.0.0.1
export WILDFLY_MANAGEMENT_BIND_ADDRESS=127.0.0.1
export SERVER_CONFIG=standalone.xml
export JBOSS_NODE_NAME=node1
export JBOSS_TX_NODE_ID=node1
export SERVER_ARGS='-Djboss.http.port=8180'
export JBOSS_MESSAGING_HOST=127.0.0.1
./target/server/bin/openshift-launch.sh > /tmp/wf.log 2>&1
```
