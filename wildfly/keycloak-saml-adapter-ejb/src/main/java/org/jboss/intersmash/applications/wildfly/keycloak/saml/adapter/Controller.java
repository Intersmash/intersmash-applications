...
public class Controller {
	...
	/**
	 * Retrieves the username of the authenticated user via an EJB lookup.
	 *
	 * @return the username retrieved from the stateless EJB, or null if not available
	 * @throws NamingException if the EJB lookup fails
	 */
	public String getUsernameEjb() throws NamingException {
		StatelessEJBLocal statelessEJB = (StatelessEJBLocal) new InitialContext()
				.lookup("java:global/ROOT/StatelessEJB!org.jboss.intersmash.applications.wildfly.keycloak.saml.adapter.StatelessEJBLocal");
		return statelessEJB.getUsername();
	}
}
