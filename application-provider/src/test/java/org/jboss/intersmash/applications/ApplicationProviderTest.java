...
class ApplicationProviderTest {
	...
	/**
	 * Verify that a provisioned server generated by the WidFly/JBoss EAP 8.x Maven plugin
	 * for the {@code wildfly-amq-broker-ssl} application
	 * exists.
	 */
	@Test
	void wildflyActiveMQArtemisProvisionedServerPath_exists() {
		Path appPath = null;
		try {
			appPath = ApplicationProvider.wildflyActiveMQArtemisProvisionedServerPath();
		} catch (Throwable e) {
			Assertions.fail(e);
		}
		assertPathExist(appPath);
	...
	/**
	 * Verify that a WAR deployment for the
	 * {@code wildfly-amq-broker-ssl} application exists.
	 */
	@Test
	void wildflyActiveMQArtemisDeploymentPath_exists() {
		Path appPath = null;
		try {
			appPath = ApplicationProvider.wildflyActiveMQArtemisDeploymentPath();
		} catch (Throwable e) {
			Assertions.fail(e);
		}
		assertPathExist(appPath);
