...
import org.jboss.intersmash.applications.ApplicationConfigurationProperties;
...
public interface WildflyApplicationConfiguration {
	...
	default String generateAdditionalMavenArgs() {
		...
		final String buildProfile = ApplicationConfigurationProperties.getWildflyApplicationBuildProfile();
		result = result.concat(StringUtils.isBlank(buildProfile) ? "" : " -Pbuild-distribution." + buildProfile);
